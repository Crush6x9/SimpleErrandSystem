<template>
  <div class="set-password-container">
    <van-nav-bar title="设置新密码" left-text="返回" left-arrow @click-left="handleBack" />
    <div class="form-container">
      <h1 class="title">设置新密码</h1>

      <van-cell-group inset>
        <van-field v-model="newPassword" :type="isPasswordVisible ? 'text' : 'password'" label="新密码"
          placeholder="请输入新密码" :right-icon="isPasswordVisible ? 'eye-o' : 'closed-eye'"
          @click-right-icon="togglePasswordVisibility" />
      </van-cell-group>

      <van-cell-group inset>
        <van-field v-model="confirmPassword" :type="isConfirmVisible ? 'text' : 'password'" label="确认密码"
          placeholder="请再次输入新密码" :right-icon="isConfirmVisible ? 'eye-o' : 'closed-eye'"
...
const verifyCode = getQueryParam(route.query.verifyCode);
